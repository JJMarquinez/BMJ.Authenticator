...
public class DeleteUserCommandHandlerTests : IAsyncLifetime
{
    ...
    public async Task ShouldDeleteUser()
    {
        ...
        var command = _testContext.GetDeleteUserCommandBuilder().WithId(userId).Build();
    ...
    {
        var command = _testContext.GetDeleteUserCommandBuilder().WithId(Guid.NewGuid().ToString()).Build();
